...
void Scene3::init(physx::PxPhysics* physics, physx::PxScene* scene)
{
	_gPhysics = physics;
	_gScene = scene;
...
void Scene3::createParticleSystem()
{
	...
	t->addForce(new GravityGenerator<Particle>(Vector3(0, -9.8, 0)));
	addParticleSystem(t);
	f = new FireworkParticleSystem(Vector3(0,0,0));
	f->addForce(new GravityGenerator<Particle>(Vector3(0, -9.8, 0)));
	addParticleSystem(f);
}
